...
export async function analyzeLinks(url: string, $: cheerio.CheerioAPI) {
  try {
    ...
    const badLinks = results
      .map(
        (r: any) => r.value || { url: 'unknown', error: r.reason, ok: false },
      )
      .filter((link) => !link.ok || link.isBotProtected);

    return {
      totalInternalLinks: uniqueLinks.length,
      brokenInternalLinks: results.filter(
        (r: any) =>
          r.status === 'fulfilled' && !r.value.ok && !r.value.isBotProtected,
      ).length,
      protectedInternalLinks: results.filter(
        (r: any) => r.status === 'fulfilled' && r.value.isBotProtected,
      ).length,
      links: badLinks,
    };
  } catch (error: any) {
